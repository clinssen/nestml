...
class TestCoCos:
    ...
    def test_invalid_element_not_defined_in_scope(self):
        ...
        assert len(Logger.get_messages(model, LoggingLevel.ERROR)) == 6
    ...
    def test_invalid_function_unique_and_defined(self):
        ...
        assert len(Logger.get_messages(model, LoggingLevel.ERROR)) == 8
    ...
    def test_invalid_divide_by_unit(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), "invalid")), "CoCoDivideByUnit.nestml"))
        assert len(Logger.get_messages(model, LoggingLevel.ERROR, message_code=MessageCode.NO_SOLITARY_PHYSICAL_UNITS)) > 0

    def test_invalid_multiply_by_unit(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), "invalid")), "CoCoMultiplyByUnit.nestml"))
        assert len(Logger.get_messages(model, LoggingLevel.ERROR, message_code=MessageCode.NO_SOLITARY_PHYSICAL_UNITS)) > 0
    ...
    def test_invalid_fractional_unit_exponent(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), "invalid")), "fractional_unit_exponent.nestml"))
        assert Logger.get_messages(model, LoggingLevel.ERROR, message_code=MessageCode.TYPE_DIFFERENT_FROM_EXPECTED)

    def test_valid_fractional_unit_exponent(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), "valid")), "fractional_unit_exponent.nestml"))
        assert len(Logger.get_messages(model, LoggingLevel.ERROR)) == 0

    def test_invalid_fractional_unit_exponent_expr(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), "invalid")), "fractional_unit_exponent_expr.nestml"))
        assert Logger.get_messages(model, LoggingLevel.ERROR, message_code=MessageCode.TYPE_DIFFERENT_FROM_EXPECTED)

    def test_valid_fractional_unit_exponent_expr(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), "valid")), "fractional_unit_exponent_expr.nestml"))
        assert len(Logger.get_messages(model, LoggingLevel.ERROR)) == 0

    def test_valid_fractional_unit_negative_exponent_expr(self):
        model = self._parse_and_validate_model(os.path.join(os.path.realpath(os.path.join(os.path.dirname(__file__), "valid")), "fractional_unit_negative_exponent_expr.nestml"))
        assert len(Logger.get_messages(model, LoggingLevel.ERROR)) == 0

    def test_invalid_fractional_unit_negative_exponent_expr(self):
    ...
    def test_valid_names_of_neurons_unique(self):
        ...
        assert len(Logger.get_messages(None, LoggingLevel.ERROR)) == 0
    ...
    def test_invalid_compound_expression_correctly_typed(self):
        ...
        assert len(Logger.get_messages(model, LoggingLevel.ERROR)) == 10
    ...
    def test_invalid_coco_kernel_type_initial_values(self):
        ...
        assert len(Logger.get_messages(model, LoggingLevel.ERROR)) == 4
