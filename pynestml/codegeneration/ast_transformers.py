...
def replace_rhs_variable(expr, variable_name_to_replace, kernel_var, spike_buf):
    def replace_kernel_var(node):
        if type(node) is ASTSimpleExpression \
            ...
            new_variable_name = construct_kernel_X_spike_buf_name(kernel_var.get_name(), spike_buf, var_order - 1, diff_order_symbol="'")
...
def replace_rhs_variables(expr, kernel_buffers):
    ...
    for kernel, spike_buf in kernel_buffers:
        for kernel_var in kernel.get_variables():
            ...
            replace_rhs_variable(expr, variable_name_to_replace=variable_name_to_replace, kernel_var=kernel_var, spike_buf=spike_buf)
...
def is_delta_kernel(kernel):
    ...
    #is_name_of_delta_kernel = type(expr) is ASTSimpleExpression \
        #and expr.is_variable() \
        #and expr.get_variable().get_scope().resolve_to_symbol(expr.get_variable().get_name(), SymbolKind.VARIABLE).is_kernel() \
        #and expr.
