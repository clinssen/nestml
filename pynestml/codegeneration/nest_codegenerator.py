...
class NESTCodeGenerator(CodeGenerator):
    ...
    def get_delta_factors_(self, neuron, equations_block):
        ...
        for ode_eq in equations_block.get_ode_equations():
            ...
            for conv_call in conv_calls:
                assert len(conv_call.args) == 2, "convolve() function call should have precisely two arguments: kernel and spike buffer"
    ...
    def generate_kernel_buffers_(self, neuron, equations_block):
        ...
        for convolve in convolve_calls:
            ...
            assert kernel is not None, "In convolution \"convolve(" + str(var.name) + ", " + str(el[1]) + ")\": no kernel by name \"" + var.get_name() + "\" found in neuron."
    ...
    def replace_convolve_calls_with_buffers_(self, neuron, equations_block, kernel_buffers):
        ...
        def replace_function_call_through_var(_expr=None):
            if _expr.is_function_call() and _expr.get_function_call().get_name() == "convolve":
                ...
                buffer_var = construct_kernel_X_spike_buf_name(var.get_name(), spike_input_port, var.get_differential_order() - 1)
    ...
    def analyse_neuron(self, neuron: ASTNeuron) -> List[ASTAssignment]:
        ...
        spike_updates = self.get_spike_update_expressions(neuron, kernel_buffers, [analytic_solver, numeric_solver], delta_factors)
        ...
        for kernel, spike_input_port in kernel_buffers:
            ...
            for kernel_var in kernel.get_variables():
                for var_order in range(get_kernel_var_order_from_ode_toolbox_result(kernel_var.get_name(), solver_dicts)):
                    kernel_spike_buf_name = construct_kernel_X_spike_buf_name(kernel_var.get_name(), spike_input_port, var_order)
                    ...
                    if not buffer_type.print_nestml_type() in ["1.", "1.0", "1", "real", "int", "bool"]:
    ...
    def transform_ode_and_kernels_to_json(self, neuron: ASTNeuron, parameters_block, kernel_buffers):
        ...
        for kernel, spike_input_port in kernel_buffers:
            ...
            for kernel_var in kernel.get_variables():
                ...
                kernel_X_spike_buf_name_ticks = construct_kernel_X_spike_buf_name(kernel_var.get_name(), spike_input_port, kernel_order, diff_order_symbol="'")
                ...
                for order in range(kernel_order):
                    iv_sym_name_ode_toolbox = construct_kernel_X_spike_buf_name(kernel_var.get_name(), spike_input_port, order, diff_order_symbol="'")
