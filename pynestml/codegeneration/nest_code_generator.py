...
def find_spiking_post_port(synapse, namespace, metadata):
    if "paired_neuron" in metadata[synapse.name].keys():
...
class NESTCodeGenerator(CodeGenerator):
    ...
    def generate_code(self,
        ...
        for synapse in synapses:
            if "neuron_synapse_pairs" in FrontendConfiguration.get_codegen_opts().keys() and "paired_neuron" in metadata[synapse.name].keys():
                post_ports = ASTUtils.get_post_ports_of_neuron_synapse_pair(metadata[synapse.name]["paired_neuron"], synapse, FrontendConfiguration.get_codegen_opts()["neuron_synapse_pairs"])
    ...
    def analyse_transform_neurons(self,
        ...
        for neuron in neurons:
            ...
            spike_updates, post_spike_updates, equations_with_delay_vars, equations_with_vector_vars = self.analyse_neuron(neuron, metadata)

            metadata[neuron.name]["spike_updates"] = spike_updates
            metadata[neuron.name]["post_spike_updates"] = post_spike_updates
            metadata[neuron.name]["equations_with_delay_vars"] = equations_with_delay_vars
            metadata[neuron.name]["equations_with_vector_vars"] = equations_with_vector_vars
    ...
    def analyse_transform_synapses(self,
        ...
        """
        for synapse in synapses:
            if not synapse.name in metadata.keys():
    ...
    def _get_synapse_model_namespace(self,
        ...
        if metadata is not None and synapse.name in metadata.keys() and "paired_neuron" in metadata[synapse.name].keys():
            # synapse is being co-generated with neuron
            paired_neuron = metadata[synapse.name]["paired_neuron"]

            namespace["paired_neuron"] = paired_neuron
        ...
        spiking_post_port = find_spiking_post_port(synapse, namespace, metadata)
    ...
    def _get_neuron_model_namespace(self,
        ...
        if metadata is not None and neuron.name in metadata.keys() and "paired_synapses" in metadata[neuron.name].keys() and metadata[neuron.name]["paired_synapses"]:
            ...
            namespace["transferred_variables"] = []
            namespace["transferred_variables_syms"] = {}
            if "transferred_variables" in metadata[neuron.name].keys():
                namespace["transferred_variables"] = [item for val_list in metadata[neuron.name]["transferred_variables"].values() for item in val_list]
                namespace["transferred_variables_syms"] = {var: neuron.get_equations_blocks()[0].get_scope().resolve_to_symbol(var, SymbolKind.VARIABLE) for var in namespace["transferred_variables"]}
            namespace["transferred_variables"] = metadata[neuron.name]["transferred_variables"]
        ...
        if namespace["uses_analytic_solver"]:
            ...
            for sym in namespace["analytic_state_variables"] + namespace["analytic_state_variables_moved"]:
                ...
                for eqn in metadata[neuron.name]["equations_with_vector_vars"]:
        ...
        if namespace["uses_numeric_solver"]:
            ...
            ASTUtils.assign_numeric_non_numeric_state_variables(neuron, numeric_state_variable_names, namespace["numeric_update_expressions"] if "numeric_update_expressions" in namespace.keys() else None, namespace["update_expressions"] if "update_expressions" in namespace.keys() else None, metadata)
        namespace["spike_updates"] = metadata[neuron.name]["spike_updates"]
    ...
    def get_spike_update_expressions(self, neuron: ASTModel, kernel_buffers, solver_dicts, delta_factors, metadata) -> Tuple[Dict[str, ASTAssignment], Dict[str, ASTAssignment]]:
        r"""
